...
----------------------------------------------------------------------------------------------------upvalue
	...
	local ui_style = {
		...
		buttonBackdropColor_Enabled = { 0.0, 0.0, 0.0, 0.75, },
		buttonBackdropColor_Disabled = { 0.25, 0.25, 0.25, 0.75, },
		buttonBackdropBorderColor = { 0.0, 0.0, 0.0, 1.0, },
		buttonHighlightColor = { 0.5, 0.5, 0.0, 0.25, },
		buttonSelectedColor = { 0.5, 0.5, 0.0, 0.5, },
		...
		explorer_width = 360,
		explorer_height = 480,
		icon_explorer = "Interface\\AddOns\\alaTradeFrame\\ARTWORK\\explorer",
	};
	...
	--[[
		alaTradeFrameSV = {
			...
			set = {
				...
				show_tradeskill_frame_price_info = bool,
				show_tradeskill_tip_craft_item_price = bool,
				show_tradeskill_tip_craft_spell_price = bool,
				show_tradeskill_tip_recipe_price = bool,
				show_tradeskill_tip_recipe_account_learned = bool,
...
do	--	LOCALE
	if LOCALE == "zhCN" or LOCALE == "zhTW" then
		...
		L["CLEAR_FILTER"] = "\124cff00ff00清除\124r";
		--
		L.SLASH_NOTE = {
		...
		L["PROFIT_FRAME_CALL_INFO1"] = "\124cffffffffEarn some money! \124r";
		...
		L["CLEAR_FILTER"] = "\124cff00ff00Clear\124r";
		--
		L.SLASH_NOTE = {
			...
			["regular_exp"] = "Search in the way of Regular Expression",
			["hide_mtsl"] = "Hide MTSL",
		};
...
do	--	MAIN
	...
	do	--	db	--	CREDIT ATLASLOOT & MTSL
		...
		do	--	PRELOAD
			function NS.SPELL_DATA_LOAD_RESULT(sid, success)
				if success then
					...
					if info and info[index_validated] then
			...
			local function preload_check_spell()
				...
				for pid, sid in ipairs(tradeskill_id) do
			...
			local function preload_check_item()
				...
				for sid, info in pairs(recipe_info) do
					if info[index_validated] then
						...
						if reagent_ids then
							for _, rid in pairs(reagent_ids) do
								if not item_info[rid] then
									RequestLoadItemDataByID(rid);
									temp_iid_list[rid] = 1;
									completed = false;
		...
		function NS.db_init()
			...
			for pid, list in pairs(recipe_sid_list_by_pid) do
	...
	do	--	func
		local function add_known_recipe(sid, GUID)
		...
		local function sub_known_recipe(sid, GUID)
			local list = explorer_hash[sid];
			if list then
				list[GUID] = nil;
			end
		...
		local function proc_stat(stat, list)
		...
		local function process_SKILL_LINES_CHANGED()
			--[[
			...
			--]]
			for pid, cpname in pairs(NS.db_table_tradeskill_check_name()) do
		...
		function NS.SKILL_LINES_CHANGED()	--	Donot process the first trigger after login. And wait for 1sec.
			C_Timer.After(1.0, function() process_SKILL_LINES_CHANGED(); NS.SKILL_LINES_CHANGED = process_SKILL_LINES_CHANGED; end);
		end
						...
						if update_list then
							...
							if update_var then
								...
								for _, sid in pairs(v) do
									sub_known_recipe(sid, PLAYER_GUID);
								end
								wipe(v);
								wipe(hash);
								for index = 1, num do
								...
								if searchText then
									...
									for index = #list, 1, -1 do
										...
										if info then
											local removed = true;
											local cid = info[index_cid];
											if strfind(NS.db_spell_name_lower(sid), searchText) or (cid and strfind(NS.db_item_link_lower(cid), searchText)) then
												removed = false;
											else
												local reagent_ids = info[index_reagents_id];
												if reagent_ids then
													for _, reagent_id in pairs(reagent_ids) do
														if strfind(NS.db_item_link_lower(reagent_id), searchText) then
															removed = false;
														end
													end
												end
			...
			if merc then
				if only_cost and frame.flag ~= 'explorer' then
					...
					for _, sid in pairs(sid_list) do
						...
						local price_a_product, price_a_material, price_a_material_known, missing = NS.price_gen_info_by_sid(sid, nMade);
		...
		function NS.process_explorer_update_list(stat, filter, searchText, list, check_hash, phase, rank, rankReversed, showKnown, showUnkown, showHighRank, donot_wipe_list)
			...
			for _, key in pairs(filter_key) do
		...
		function NS.process_explorer_update(frame, update_list)
			if frame:IsVisible() then
				...
				if update_list then
					...
					NS.process_explorer_update_list(explorer_stat_list, set.filter, set.searchText, 
	...
	do	--	hook ui
		--	list button handler
			...
			local function button_OnEnter(self)
				...
				if pid then
					...
					if info then
						GameTooltip:SetOwner(self, "ANCHOR_RIGHT");
						if SET[pid].showItemInsteadOfSpell and info[index_cid] then
							GameTooltip:SetItemByID(info[index_cid]);
						else
							GameTooltip:SetSpellByID(info[index_sid]);
						end
						local phase = NS.db_get_phase_by_sid(sid);
					...
					if onEnter_identity >= BIG_NUMBER then
						onEnter_identity = 1;
					end
			...
			local function enchant_link(sid)
				...
				if button == "LeftButton" then
					if IsShiftKeyDown() then
						...
						if cid then
							...
							ChatEdit_InsertLink(enchant_link(sid), ADDON);
						end
					elseif IsAltKeyDown() then
						...
						if data then
							...
							if n and n > 0 then
								...
								if m1 == m2 then
									text1 = frame.recipe_link(data) .. "x" .. m1 .. L["Needs: "];
								else
									text1 = frame.recipe_link(data) .. "x" .. m1 .. "-" .. m2 .. L["Needs: "];
								end
								text2 = "";
								if n > 4 then
									for i = 1, n do
							...
							if info then
								if cid then
									text1 = (NS.db_item_link(cid) or L["Unk"] .. cid) .. "x" .. (info[index_num_made_min] == info[index_num_made_max] and info[index_num_made_min] or (info[index_num_made_min] .. "-" .. info[index_num_made_max])) .. L["Needs: "];
								else
									text1 = (NS.db_spell_name(sid) or L["Unk"] .. sid) .. L["Needs: "];
								end
								text2 = "";
								local rinfo = info[index_reagents_id];
								if #rinfo > 4 then
									for i = 1, #rinfo do
										text2 = text2 .. (NS.db_item_name(rinfo[i]) or L["Unk"] .. rinfo[i]) .. "x" .. info[index_reagents_count][i];
									end
								else
									for i = 1, #rinfo do
										text2 = text2 .. (NS.db_item_link(rinfo[i]) or L["Unk"] .. rinfo[i]) .. "x" .. info[index_reagents_count][i];
									end
								end
							end
						end
					...
					elseif IsControlKeyDown() then
						...
						if data and type(data) == 'number' then
							...
							local scroll = frame.hooked_scroll;
							local num = frame.recipe_num();
		...
		--
		local function funcToCreateProfitButton(parent, index, buttonHeight)
			local button = CreateFrame("BUTTON", nil, parent);
			button:SetHeight(buttonHeight);
			button:SetBackdrop(ui_style.buttonBackdrop);
			button:SetBackdropColor(unpack(ui_style.buttonBackdropColor_Enabled));
			...
			glow:SetVertexColor(unpack(ui_style.buttonSelectedColor));
		...
		local function funcToSetProfitButton(button, data_index)
			...
			if data_index <= #list then
				...
				if data then
					if frame.flag == 'explorer' then
						...
						button.title:SetText(NS.db_spell_name(sid));
						...
						if name and rank ~= 'header' then
							...
							button.title:SetTextColor(unpack(rank_color[rank_index[rank]]));
							button.note:SetText(merc.MoneyString(list[data_index][2]));
							if quality then
								local r, g, b, code = GetItemQualityColor(quality);
								button.quality_glow:SetVertexColor(r, g, b);
					...
					if SET.colored_rank_for_unknown and frame.flag ~= 'explorer' then
						button:SetBackdropColor(unpack(ui_style.buttonBackdropColor_Disabled));
					else
						button:SetBackdropColor(unpack(ui_style.buttonBackdropColor_Enabled));
					end
					local _, quality, icon;
					...
					if SET.colored_rank_for_unknown and frame.flag ~= 'explorer' then
						...
						local var = rawget(VAR, pid);
						local cur_rank = var and var.cur_rank or 0;
						button.title:SetTextColor(unpack(rank_color[NS.db_get_difficulty_rank_by_sid(sid, cur_rank)]));
					else
		...
		local function funcToCreateListButton(parent, index, buttonHeight)
			local button = CreateFrame("BUTTON", nil, parent);
			button:SetHeight(buttonHeight);
			button:SetBackdrop(ui_style.buttonBackdrop);
			button:SetBackdropColor(unpack(ui_style.buttonBackdropColor_Enabled));
		...
		local function funcToSetListButton(button, data_index)
			local frame = button.frame;
			local list = button.list;
			local hash = frame.hash;
			if data_index <= #list then
				...
				if pid then
					...
					if data then
						...
						local name = NS.db_spell_name(sid);
						button.icon:SetTexture(icon);
						button.icon:SetVertexColor(1.0, 0.0, 0.0, 1.0);
						button.title:SetText(name);
						if SET.colored_rank_for_unknown then
							button.title:SetTextColor(unpack(rank_color[NS.db_get_difficulty_rank_by_sid(sid, VAR[pid].cur_rank or 0)]));
						else
							button.title:SetTextColor(1.0, 0.0, 0.0, 1.0);
						end
						if set.showRank then
		...
		local function change_set_without_update(set, key, val)
			set[key] = val;
		end
		local function change_set_with_update(set, key, val)
			change_set_without_update(set, key, val);
			set.update = true;
		end
		function NS.ui_hook(meta)
			meta.update_func = _G[meta.update_func_name];
			meta.clear_filter();
			for _, name in pairs(meta.inoperative_name) do
				meta.inoperative_func[name] = _G[name];
				_G[name] = _noop_;
			end

			local hooked = meta.hooked_frame;
			...
			local scroll = ALASCR(frame, nil, nil, ui_style.button_height, funcToCreateListButton, funcToSetListButton);
			scroll:SetPoint("BOTTOMLEFT", 4, 2);
			scroll:SetPoint("TOPRIGHT", - 4, - 22);
			frame.scroll = scroll;

			local button = CreateFrame("BUTTON", nil, hooked, "UIPanelButtonTemplate");
			button:SetSize(70, 20);
			button:SetPoint("RIGHT", meta.normal_anchor_top, "LEFT", - 2, 0);
			...
			do	--	tab
				...
				tabFrame:SetHeight(ui_style.tab_size + ui_style.tab_interval * 2);
				...
				function tabFrame:CreateTab(index)
					...
					tab:SetSize(ui_style.tab_size, ui_style.tab_size);
					...
					tabs[index] = tab;
					if index == 1 then
						tab:SetPoint("LEFT", tabFrame, "LEFT", ui_style.tab_interval, 0);
					else
						tab:SetPoint("LEFT", tabs[index - 1], "RIGHT", ui_style.tab_interval, 0);
					end
					return tab;
				end
				function tabFrame:SetNumTabs(num)
					...
					tabFrame:SetWidth(ui_style.tab_size * num + ui_style.tab_interval * (num + 1));
				end
				function tabFrame:SetTab(index, pname, ptexture)
					local tab = self.tabs[index] or self:CreateTab(index);
					tab:Show();
					tab.pname = pname;
				...
				function tabFrame:Update()
					...
					numSkill = numSkill + 1;
					self:SetTab(numSkill, 'explorer', ui_style.icon_explorer);
			...
			do	-- search_box
				...
				searchEditOK:SetPoint("TOPRIGHT", frame, "TOPRIGHT", - 46, - 4);
				...
				function frame:Search(text)
					...
					if pid then
						if text == "" then
							change_set_with_update(SET[pid], "searchText", nil);
							if not searchEdit:HasFocus() then
								searchEditNote:Show();
							end
							searchCancel:Hide();
						else
							change_set_with_update(SET[pid], "searchText", text);
							searchCancel:Show();
							searchEditNote:Hide();
						end
					end
			...
			do	-- I feel lucky
				...
				call:SetPoint("TOPRIGHT", frame, "TOPRIGHT", - 24, -4);
				...
				local scroll = ALASCR(profitFrame, nil, nil, ui_style.button_height, funcToCreateProfitButton, funcToSetProfitButton);
				...
				close:SetScript("OnClick", function()
					local pid = NS.db_get_pid_by_pname(frame.pname());
					if pid then
						change_set_without_update(SET[pid], "showProfit", false);
					end
					profitFrame:Hide();
				end);
				...
				call:SetScript("OnClick", function(self)
					if merc then
						...
						if profitFrame:IsShown() then
							...
							profitFrame:Show();
							if pid then
								change_set_without_update(SET[pid], "showProfit", true);
							end
						end
					end
				end);
			...
			do	-- set_frame
				...
				setFrame:SetBackdrop(ui_style.frameBackdrop);
				setFrame:SetBackdropColor(unpack(ui_style.frameBackdropColor));
				setFrame:SetBackdropBorderColor(unpack(ui_style.frameBackdropBorderColor));
				setFrame:SetFrameStrata("HIGH");
				setFrame:SetHeight(48);
				setFrame:SetPoint("LEFT", frame);
				setFrame:SetPoint("RIGHT", frame);
				setFrame:SetPoint("BOTTOM", frame.tabFrame, "TOP", 0, 2);
				setFrame:Hide();
				frame.setFrame = setFrame;

				local call = CreateFrame("BUTTON", nil, frame);
				call:SetSize(16, 16);
				call:SetNormalTexture("interface\\buttons\\ui-optionsbutton");
				call:SetPushedTexture("interface\\buttons\\ui-optionsbutton");
				call:GetPushedTexture():SetVertexColor(0.5, 0.5, 0.5, 0.5);
				call:SetHighlightTexture("interface\\buttons\\ui-optionsbutton");
				call:GetHighlightTexture():SetVertexColor(0.5, 0.5, 0.5, 0.5);
				call:SetPoint("TOPRIGHT", frame, "TOPRIGHT", - 4, - 4);
				call:SetScript("OnClick", function(self)
					local pid = NS.db_get_pid_by_pname(frame.pname());
					if setFrame:IsShown() then
						...
						if pid then
							change_set_without_update(SET[pid], "showSet", false);
						end
					else
						setFrame:Show();
						self:GetNormalTexture():SetVertexColor(0.75, 0.75, 0.75, 1.0);
						if pid then
							change_set_without_update(SET[pid], "showSet", true);
						end
					end
				end);

				local checkBoxes = {  };
				for index, key in pairs({ "showUnkown", "showKnown", "showHighRank", "showItemInsteadOfSpell", "showRank", "haveMaterials", }) do
					...
					if index % 3 == 1 then
						if index == 1 then
							check:SetPoint("TOPLEFT", 4, 0);
						else
							check:SetPoint("TOPLEFT", checkBoxes[index - 3], "BOTTOMLEFT", 0, 0);
						end
					else
						check:SetPoint("LEFT", checkBoxes[index - 1], "RIGHT", 78, 0);
					end
					if index == 1 or index == 2 or index == 3 or index == 6 then
						check:SetScript("OnClick", function(self)
							local pid = NS.db_get_pid_by_pname(frame.pname());
							if pid then
								change_set_with_update(SET[pid], key, self:GetChecked());
							end
							frame.update_func();
						end);
					else
				...
				phaseSlider:HookScript("OnValueChanged", function(self, value, userInput)
					if userInput then
						local pid = NS.db_get_pid_by_pname(frame.pname());
						if pid then
							change_set_with_update(SET[pid], "phase", value);
							frame.update_func();
						end
					end
				...
				function frame:RefreshSetFrame()
					...
					if pid then
						local set = SET[pid];
						for index, check in pairs(checkBoxes) do
		...
		----
		local function price_info_func(frame, need_update)
			if merc then
				local sid = frame.selected_sid;
				local price_info = frame.price_info;
				...
				local nMade, minMade, maxMade = NS.db_get_num_made_by_sid(sid);
				local price_a_product, _, price_a_material, unk_in = NS.price_gen_info_by_sid(sid, nMade);
		...
		local function set_price_info_func(frame, ...)
			local select_time = 0;
			local prev_sid = nil;
			local function update()
				if GetTime() - select_time > 0.4 then
					price_info_func(frame, true);
				end
			end
			local function execute()
				...
				if SET.show_tradeskill_frame_price_info and merc then
					if GetTime() - select_time > 0.09 then
						select_time = GetTime();
						C_Timer.After(0.5, update);
					end
				end
			end
			hooksecurefunc(frame, "select", execute);
			hooksecurefunc(frame, "update_func", execute);
			for i = 1, select("#", ...) do
				hooksecurefunc(select(i, ...), execute);
			end
			frame.execute_price_info_func = execute;
			if merc and merc.add_cache_callback then
				merc.add_cache_callback(execute);
			end
		end
		function NS.hook_Blizzard_TradeSkillUI(addon)
			local meta = {
				hooked_frame = TradeSkillFrame,
				hooked_scroll = TradeSkillListScrollFrameScrollBar,
				...
				update_func = TradeSkillFrame_Update,
			...
			gui[addon] = frame;
			do	-- rank & price_info in DetailScroll
				local price_info = {  };
				frame.price_info = price_info;
				price_info[1] = TradeSkillDetailScrollChildFrame:CreateFontString(nil, "OVERLAY");
				price_info[1]:SetFont(GameFontNormal:GetFont());
				...
				price_info[2]:SetFont(GameFontNormal:GetFont());
				...
				price_info[3]:SetFont(GameFontNormal:GetFont());
				price_info[3]:SetPoint("TOPLEFT", price_info[2], "BOTTOMLEFT", 0, 0);
				price_info[3]:Hide();
				set_price_info_func(frame, "TradeSkillFrame_SetSelection");

				local note = meta.relative_note:CreateFontString(nil, "OVERLAY");	-- rank color
				note:SetFont(GameFontNormal:GetFont());
				note:SetPoint("TOPLEFT", TradeSkillDetailScrollChildFrame, "TOPLEFT", 5, -50);
				frame.note_in_frame = note;
				local function hook()
					note:SetText(NS.db_get_difficulty_rank_list_text_by_sid(frame.selected_sid));
				end
				hooksecurefunc(meta.update_func_name, hook);
				hooksecurefunc(frame, "select", hook);
				hooksecurefunc(frame, "update_func", hook);

				function NS.ui_toggle_price_info_TradeSkillFrame(on)
					if on then
						price_info[1]:Show();
						price_info[2]:Show();
						price_info[3]:Show();
						price_info[1]:ClearAllPoints();
						if SET.show_tradeskill_frame_rank then
							price_info[1]:SetPoint("TOPLEFT", note, "BOTTOMLEFT", 0, -3);
						else
							price_info[1]:SetPoint("TOPLEFT", TradeSkillDetailScrollChildFrame, "TOPLEFT", 5, -50);
						end
						TradeSkillReagentLabel:ClearAllPoints();
						TradeSkillReagentLabel:SetPoint("TOPLEFT", price_info[3], "BOTTOMLEFT", 0, -3);
						frame.execute_price_info_func();
					else
						price_info[1]:Hide();
						price_info[2]:Hide();
						price_info[3]:Hide();
						TradeSkillReagentLabel:ClearAllPoints();
						if SET.show_tradeskill_frame_rank then
							TradeSkillReagentLabel:SetPoint("TOPLEFT", note, "BOTTOMLEFT", 0, -3);
						else
							TradeSkillReagentLabel:SetPoint("TOPLEFT", TradeSkillDetailScrollChildFrame, "TOPLEFT", 5, -50);
						end
					end
				end
				function NS.ui_toggle_rank_TradeSkillFrame(on)
					if on then
						note:Show();
						-- note:ClearAllPoints();
						-- note:SetPoint("TOPLEFT", TradeSkillDetailScrollChildFrame, "TOPLEFT", 5, -50);
						TradeSkillReagentLabel:ClearAllPoints();
						if SET.show_tradeskill_frame_price_info then
							price_info[1]:ClearAllPoints();
							price_info[1]:SetPoint("TOPLEFT", note, "BOTTOMLEFT", 0, -3);
							TradeSkillReagentLabel:SetPoint("TOPLEFT", price_info[3], "BOTTOMLEFT", 0, -3);
						else
				...
				NS.ui_toggle_price_info_TradeSkillFrame(SET.show_tradeskill_frame_price_info);
				NS.ui_toggle_rank_TradeSkillFrame(SET.show_tradeskill_frame_rank);
			end
			do	-- mouse click
				for i = 1, 8 do
		...
		function NS.hook_Blizzard_CraftUI(addon)
			local meta = {
				hooked_frame = CraftFrame,
				hooked_scroll = CraftListScrollFrameScrollBar,
			...
			do	-- rank & price_info in DetailScroll
				...
				price_info[2]:Hide();
				set_price_info_func(frame, "CraftFrame_SetSelection");

				local note = meta.relative_note:CreateFontString(nil, "OVERLAY");	-- rank color
				note:SetFont(GameFontNormal:GetFont());
				note:SetPoint("TOPLEFT", CraftDetailScrollChildFrame, "TOPLEFT", 5, -50);
				frame.note_in_frame = note;
				local function hook()
					note:SetText(NS.db_get_difficulty_rank_list_text_by_sid(frame.selected_sid));
				end
				...
				function NS.ui_toggle_price_info_CraftFrame(on)
					if on then
						price_info[2]:Show();
						price_info[2]:ClearAllPoints();
						if SET.show_tradeskill_frame_rank then
							price_info[2]:SetPoint("TOPLEFT", note, "BOTTOMLEFT", 0, -3);
						else
							price_info[2]:SetPoint("TOPLEFT", CraftDetailScrollChildFrame, "TOPLEFT", 5, -50);
						end
						...
						CraftDescription:ClearAllPoints();
						if SET.show_tradeskill_frame_rank then
							CraftDescription:SetPoint("TOPLEFT", note, "BOTTOMLEFT", 0, -3);
						else
							CraftDescription:SetPoint("TOPLEFT", CraftDetailScrollChildFrame, "TOPLEFT", 5, -50);
						end
					end
				end
				function NS.ui_toggle_rank_CraftFrame(on)
					if on then
						note:Show();
						-- note:ClearAllPoints();
						-- note:SetPoint("TOPLEFT", CraftDetailScrollChildFrame, "TOPLEFT", 5, -50);
				...
				NS.ui_toggle_price_info_CraftFrame(SET.show_tradeskill_frame_price_info);
				NS.ui_toggle_rank_CraftFrame(SET.show_tradeskill_frame_rank);
			end
			do	-- mouse click
				for i = 1, 8 do
			...
			do	-- auto filter recipe when trading
				...
				_EventHandler:SetScript("OnEvent", function(self, event, _1)
		...
		----
		local function funcToCreateExplorerButton(parent, index, buttonHeight)
			local button = CreateFrame("BUTTON", nil, parent);
			button:SetHeight(buttonHeight);
			button:SetBackdrop(ui_style.buttonBackdrop);
			button:SetBackdropColor(unpack(ui_style.buttonBackdropColor_Enabled));
		...
		local function funcToSetExplorerButton(button, data_index)
			local frame = button.frame;
			local list = button.list;
			local hash = frame.hash;
			if data_index <= #list then
		...
		--	set button handler
			...
			local function setButton_OnClick(self)
				local key = self.key;
				local set = SET.explorer;
				...
				if filter[key] then
					...
					NS.process_explorer_update_list(temp_stat_list, temp_filter, set.searchText, 
				...
				wipe(elements);
				elements[1] = { text = L["CLEAR_FILTER"], para = { frame, key, nil, }, };
		...
		function NS.ui_CreateExplorer()
			local frame = CreateFrame("FRAME", "ALA_TRADESKILL_EXPLORER_UI", UIParent);
			tinsert(UISpecialFrames, "ALA_TRADESKILL_EXPLORER_UI");
			frame:SetBackdrop(ui_style.frameBackdrop);
			frame:SetBackdropColor(unpack(ui_style.frameBackdropColor));
			frame:SetBackdropBorderColor(unpack(ui_style.frameBackdropBorderColor));
			frame:SetSize(ui_style.explorer_width, ui_style.explorer_height);
			frame:SetFrameStrata("HIGH");
			...
			local scroll = ALASCR(frame, nil, nil, ui_style.button_height, funcToCreateExplorerButton, funcToSetExplorerButton);
			scroll:SetPoint("BOTTOMLEFT", 4, 4);
			...
			do	-- search_box
				...
				function frame:Search(text)
					if text == "" then
						SET.explorer.searchText = nil;
						if not searchEdit:HasFocus() then
							searchEditNote:Show();
						end
						searchCancel:Hide();
					else
						SET.explorer.searchText = text;
						searchCancel:Show();
						searchEditNote:Hide();
					end
					frame.update_func();
				...
				function frame:RefreshSearchEdit()
					local searchText = SET.explorer.searchText or "";
					if searchEdit:GetText() ~= searchText then
						searchEdit:SetText(searchText);
					end
					if searchText == "" then
			...
			do	-- set_frame
				...
				local checkBoxes = {  };
				for index, key in pairs({ "showUnkown", "showKnown", "showItemInsteadOfSpell", "showRank", }) do
					local check = CreateFrame("CHECKBUTTON", nil, setFrame, "OptionsBaseCheckButtonTemplate");
					check:SetSize(24, 24);
					check:SetHitRectInsets(0, 0, 0, 0);
					check:Show();
					...
					if index % 4 == 1 then
						...
						check:SetPoint("LEFT", checkBoxes[index - 1], "RIGHT", 70, 0);
					end
					if index <= 2 then
						check:SetScript("OnClick", function(self)
				...
				local dropDowns = {  };
				for index, key in pairs({ "skill", "type", "subType", "eqLoc", }) do
					...
					if index % 4 == 1 then
						if index == 1 then
							drop:SetPoint("TOPLEFT", checkBoxes[1], "BOTTOMLEFT", 4, 0);
						else
							drop:SetPoint("TOPLEFT", dropDowns[index - 3], "BOTTOMLEFT", 0, 0);
						end
					else
						drop:SetPoint("LEFT", dropDowns[index - 1], "RIGHT", 70, 0);
					end
					drop:SetScript("OnClick", setButton_OnClick);
					drop.frame = frame;
				end
	...
	do	-- 	tradeskill tooltip
		...
		local n_space = 4;
		function NS.price_gen_info_by_sid(sid, num, lines, stack_level, is_enchanting)	--	merc not checked
			local info = NS.db_get_info_by_sid(sid);
			if not info then
				return;
			end
			...
			if stack_level <= 4 and (stack_level == 0 or not recipe_black_list_sid[sid]) then
				...
				for i = 1, #reagid do
					...
					if nsids > 0 then
						for index, sid in ipairs(sids) do
							if NS.db_get_pid_by_sid(sid) == pid then
								local p2, c2 = NS.price_gen_info_by_sid(sid, num, detail_lines, stack_level + 1);
								p = p or p2;
								if c2 then
					...
					if not got then
						local name = merc.query_name_by_id(iid) or NS.db_item_name(iid) or (L["UNK_ITEM_ID"] .. iid) or "";
						...
						if p then
							...
							if detail_lines then
								tinsert(detail_lines, "\124cff000000*\124r" .. strrep(" ", stack_level * n_space) .. name .. "x" .. num);
			...
			local name = cid and (merc.query_name_by_id(cid) or NS.db_item_name(cid) or (L["UNK_ITEM_ID"] .. cid)) or "";
			...
			if stack_level == 0 then
				if lines then
					...
					if is_enchanting then
						if cost then
							tinsert(lines, "\124cffff7f00*\124r" .. "\124cffffffff" .. NS.db_spell_name(sid) .. "\124r" or L["COST_PRICE"]);
				...
				if price and (not cost or cost >= price) then
					if lines then
						tinsert(lines, "\124cff000000*\124r" .. strrep(" ", (stack_level - 1) * n_space) .. name .. "x" .. num);
		...
		local function set_tip_by_sid(tip, sid)
			...
			local detail_lines = {  };
			NS.price_gen_info_by_sid(sid, NS.db_get_num_made_by_sid(sid), detail_lines, 0, cid == nil);
		...
		local function set_tip_by_cid(tip, cid)
			...
			if nsids > 0 then
				...
				for index, sid in ipairs(sids) do
					...
					tip:AddLine("\124cff00afff" .. name .. "\124r");
					local detail_lines = {  };
					NS.price_gen_info_by_sid(sid, NS.db_get_num_made_by_sid(sid), detail_lines, 0, false);
					if #detail_lines > 0 then
						for i = 1, #detail_lines, 2 do
							tip:AddDoubleLine(detail_lines[i], detail_lines[i + 1]);
						end
					...
					for GUID, VAR in pairs(alaTradeFrameSV.var) do
						-- if PLAYER_GUID ~= GUID then
							...
							if var then
								...
								if var[2][sid] then
									name = L["RECIPE_LEARNED"] .. " " .. name .. "\124cffffffff" .. var.cur_rank .. "/" .. var.max_rank .. "\124r";
									tip:AddLine(name);
								else
									if var.cur_rank >= learn_rank then
										name = L["RECIPE_NOT_LEARNED"] .. " " .. name .. "\124cffffffff" .. var.cur_rank .. "/" .. var.max_rank .. "\124r";
									else
										name = L["RECIPE_NOT_LEARNED"] .. " " .. name .. "\124cffffffff" .. var.cur_rank .. "\124r\124cffffffff/" .. var.max_rank .. "\124r";
									end
									tip:AddLine(name);
								end
							end
		...
		local function hook_tooltip_hyperlink(tip, link)
			if merc then
		...
		local function hook_tooltip_spell_by_id(tip, sid)
			if merc then
				if SET.show_tradeskill_tip_craft_spell_price then
		...
		local function hook_tooltip_item_by_id(tip, cid)
			if merc then
				if SET.show_tradeskill_tip_recipe_account_learned then
					local sid = NS.db_get_sid_by_rid(cid);
					if sid then
		...
		local function hook_tooltip_spell(tip)
			local _, sid = tip:GetSpell();
			if sid then
				hook_tooltip_spell_by_id(tip, sid);
			end
		end
		local function hook_tooltip_item(tip)
			local _, link = tip:GetItem();
			local cid = link and tonumber(select(3, strfind(link, "item:(%d+)")) or nil);
			if cid then
				hook_tooltip_item_by_id(tip, cid);
			end
		end
		local function hook_tooltip(tip)
			local _, sid = tip:GetSpell();
			if sid then
				hook_tooltip_spell_by_id(tip, sid);
			else
		...
		function NS.hook_tooltip()
			hooksecurefunc(GameTooltip, "SetHyperlink", hook_tooltip_hyperlink);
			hooksecurefunc(ItemRefTooltip, "SetHyperlink", hook_tooltip_hyperlink);
			hooksecurefunc(GameTooltip, "SetSpellByID", hook_tooltip_spell_by_id);
			hooksecurefunc(ItemRefTooltip, "SetSpellByID", hook_tooltip_spell_by_id);
			hooksecurefunc(GameTooltip, "SetItemByID", hook_tooltip_item_by_id);
			hooksecurefunc(ItemRefTooltip, "SetItemByID", hook_tooltip_item_by_id);
			hooksecurefunc(GameTooltip, "SetCraftSpell", hook_tooltip);
			hooksecurefunc(ItemRefTooltip, "SetCraftSpell", hook_tooltip);
			hooksecurefunc(GameTooltip, "SetMerchantItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetBuybackItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetBagItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetAuctionItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetAuctionSellItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetLootItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetLootRollItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetInventoryItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetTradePlayerItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetTradeTargetItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetQuestItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetQuestLogItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetInboxItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetSendMailItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetTradeSkillItem", hook_tooltip_item);
			hooksecurefunc(GameTooltip, "SetCraftItem", hook_tooltip_item);
		end
	end
	...
	do	--	check cooldown
		...
		for pid, list in pairs(cooldown_list) do
	...
	do	--	BOARD
		...
		local function calendar_head(sid)			--	tex, coord, title, color
			return NS.db_get_texture_by_pid(NS.db_get_pid_by_sid(sid)), nil, NS.db_spell_name(sid), nil;
		end
						...
						if NS.db_is_pid(pid) then
							...
							if cool and tnotempty(cool) then
								...
								for sid, c in pairs(cool) do
									...
									local sname = "\124T" .. (texture or ui_style.texture_unk) .. ":12:12:0:0\124t " .. (NS.db_spell_name(sid) or GetSpellInfo(sid) or ("spellId:" .. sid));
									if c > 0 then
		...
		function NS.ui_CreateBoard(_SVAR, set)
			SVAR = _SVAR;
			bset = set;
			...
			function board:Update()
				board:SetHeight(16 * max(self.curLine, 1));
			end
			C_Timer.NewTicker(1.0, update_func);
			bset = alaTradeFrameSV.set.board;
			if bset.shown then
				board:Show();
			else
				board:Hide();
			...
			if bset.pos then
				...
				board:SetPoint(unpack(bset.pos));
			end
		end
		function NS.ui_toggle_board(on)
			if on then
				show();
			else
				hide();
						...
						if sid then
							...
							if sid  then
								...
								if cid then
									...
									if ok then
										print((NS.db_item_link(cid) or "") .. output);
									end
								end
							end
						end
	...
	do	--	mtsl
		local function toggle_mtsl(hide)
			if hide then
	...
	function NS.init_createGUI()
		----	hook TradeSkillUI CraftUI
			if IsAddOnLoaded("Blizzard_TradeSkillUI") then
				NS.hook_Blizzard_TradeSkillUI("Blizzard_TradeSkillUI");
			else
				...
				-- NS.add_AddonLoadedHandler("Blizzard_TradeSkillUI", NS.hook_Blizzard_TradeSkillUI);
			end
			if IsAddOnLoaded("Blizzard_CraftUI") then
				NS.hook_Blizzard_CraftUI("Blizzard_CraftUI");
			else
				local f = CreateFrame("FRAME");
				f:RegisterEvent("ADDON_LOADED");
				f:SetScript("OnEvent", function(self, event, addon)
				...
				-- NS.add_AddonLoadedHandler("Blizzard_CraftUI", NS.hook_Blizzard_CraftUI);
			end
		----
		gui["Explorer"] = NS.ui_CreateExplorer();
	end
	--
	...
	function NS.ON_SET_CHANGED(key, val)
		if key == 'show_tradeskill_frame_price_info' then
			if NS.ui_toggle_price_info_TradeSkillFrame then
				NS.ui_toggle_price_info_TradeSkillFrame(val);
			end
			if NS.ui_toggle_price_info_CraftFrame then
				NS.ui_toggle_price_info_CraftFrame(val);
			end
			NS.ui_update_all();
		elseif key == 'show_tradeskill_frame_rank' then
			if NS.ui_toggle_rank_TradeSkillFrame then
				NS.ui_toggle_rank_TradeSkillFrame(val);
			end
			if NS.ui_toggle_rank_CraftFrame then
				NS.ui_toggle_rank_CraftFrame(val);
			end
			NS.ui_update_all();
...
do	--	INITIALIZE
	...
	local function MODIFY_SAVED_VARIABLE()
		if alaTradeFrameSV then
			...
			if alaTradeFrameSV._version < 200404.0 then
				local old = alaTradeFrameSV;
				_G.alaTradeFrameSV = { set = {  }, var = {  }, };
				for pid, sv in pairs(old) do
					if NS.db_is_pid(pid) then
						local set = {  };
						for k, v in pairs(sv) do
							if type(k) == 'number' then
								local nsids, sids = NS.db_get_sid_by_pid_cid(pid, k);
								if nsids > 0 then
									for i = 1, nsids do
										set[sids[i]] = 1;
									end
						...
						alaTradeFrameSV.set[pid] = set;
					end
				end
				alaTradeFrameSV._version = 200404.0;
			end
			if alaTradeFrameSV._version < 200408.0 then
				alaTradeFrameSV.set.show_tradeskill_tip_craft_item_price = alaTradeFrameSV.set.tradeskill_crafted_item_price;
				alaTradeFrameSV.set.tradeskill_crafted_item_price = nil;
			end
			if alaTradeFrameSV._version < 200414.0 then
				for GUID, VAR in pairs(alaTradeFrameSV.var) do
					for pid, var in pairs(VAR) do
						if NS.db_is_pid(pid) then
							var.realm_name = nil;
					...
					local _, _, realm_id = strfind(GUID, "Player%-(.+)%-");
					realm_id = tonumber(realm_id);
					if realm_id then
						VAR.realm_id = realm_id;
						if realm_id == PLAYER_REALM_ID then
							VAR.realm_name = PLAYER_REALM_NAME;
						else
							alaTradeFrameSV.var[GUID] = nil;
						end
					end
				end
				alaTradeFrameSV.set.show_tradeskill_crafted_item_price = nil;
				alaTradeFrameSV.set.show_tradeskill_tip_price = nil;
				alaTradeFrameSV.set.show_tradeskill_recipe_price = nil;
				alaTradeFrameSV.set.explorer = default_explorer_set;
				if alaTradeFrameSV.set.board then
					alaTradeFrameSV.set.board.shown = alaTradeFrameSV.set.board.shown or alaTradeFrameSV.set.board.on;
					alaTradeFrameSV.set.board.on = nil;
				else
					alaTradeFrameSV.set.board = { shown = true, locked = false, };
				end
				alaTradeFrameSV.board = nil;
				local fav = alaTradeFrameSV.fav;
				if fav == nil then
					fav = {  };
					alaTradeFrameSV.fav = fav;
				end
				for pid, set in pairs(alaTradeFrameSV.set) do
					if NS.db_is_pid(pid) then
						set.showConfig = nil;
			...
			if alaTradeFrameSV._version < 200419.1 then
				for GUID, VAR in pairs(alaTradeFrameSV.var) do
					for pid, var in pairs(VAR) do
						if NS.db_is_pid(pid) then
							if var.cur_rank == nil or type(var.cur_rank) ~= "number" then
								var.cur_rank = "-";
							end
							if var.max_rank == nil or type(var.max_rank) ~= "number" then
								var.max_rank = "-";
							end
						end
					end
					if VAR.realm_id == nil or VAR.realm_name == nil then
						local _, _, realm_id = strfind(GUID, "Player%-(.+)%-");
						realm_id = tonumber(realm_id);
						if realm_id then
							VAR.realm_id = realm_id;
				...
				alaTradeFrameSV.set.colored_rank_for_unknown = (alaTradeFrameSV.set.colored_rank_for_unknown == nil) and false;
				alaTradeFrameSV.set.explorer.costOnly = nil;
				alaTradeFrameSV.cmm = {  };
			end
			if alaTradeFrameSV._version < 200501.3 then
				wipe(alaTradeFrameSV.cmm);
			end
			if alaTradeFrameSV._version < 200505.1 then
				for pid, set in pairs(alaTradeFrameSV.set) do
					if NS.db_is_pid(pid) then
						set.haveMaterials = default_set.haveMaterials;
						set.show_tradeskill_frame_rank = nil;
						set.show_tradeskill_frame_price_info = nil;
						set.show_tradeskill_tip_craft_spell_price = nil;
						set.show_tradeskill_tip_craft_item_price = nil;
						set.show_tradeskill_tip_recipe_price = nil;
						set.show_tradeskill_tip_recipe_account_learned = nil;
						set.regular_exp = nil;
						set.hide_mtsl = nil;
					end
				end
				alaTradeFrameSV.set.show_tradeskill_frame_rank = true;
				alaTradeFrameSV.set.show_tradeskill_frame_price_info = true;
				alaTradeFrameSV.set.show_tradeskill_tip_craft_spell_price = true;
				alaTradeFrameSV.set.show_tradeskill_tip_craft_item_price = true;
				alaTradeFrameSV.set.show_tradeskill_tip_recipe_price = true;
				alaTradeFrameSV.set.show_tradeskill_tip_recipe_account_learned = true;
				alaTradeFrameSV.set.regular_exp = false;
				alaTradeFrameSV.set.hide_mtsl = false;
			end
		else
			_G.alaTradeFrameSV = {
				set = {
					show_tradeskill_frame_rank = true,
		...
		alaTradeFrameSV._version = 200505.0;
		...
		if AVAR[PLAYER_GUID] == nil then
			AVAR[PLAYER_GUID] = { realm_id = PLAYER_REALM_ID, realm_name = PLAYER_REALM_NAME, };
		end
		...
		do
			...
			-- if SET.show_tradeskill_frame_rank == nil then SET.show_tradeskill_frame_rank = _protected; end
			-- if SET.show_tradeskill_frame_price_info == nil then SET.show_tradeskill_frame_price_info = _protected; end
			-- if SET.show_tradeskill_tip_craft_spell_price == nil then SET.show_tradeskill_tip_craft_spell_price = _protected; end
			-- if SET.show_tradeskill_tip_craft_item_price == nil then SET.show_tradeskill_tip_craft_item_price = _protected; end
			-- if SET.show_tradeskill_tip_recipe_price == nil then SET.show_tradeskill_tip_recipe_price = _protected; end
			-- if SET.show_tradeskill_tip_recipe_account_learned == nil then SET.show_tradeskill_tip_recipe_account_learned = _protected; end
		end
	end
	...
	local function init()
		...
		NS.db_init();		--	!!!must be run earlier than any others!!!
		MODIFY_SAVED_VARIABLE();
		NS.process_init_known_recipe_hash();
		NS.init_regEvent();
		NS.init_createGUI();
		NS.hook_tooltip();
		NS.ui_CreateBoard(AVAR, alaTradeFrameSV.set.board);
...
do	--	SLASH
	...
	local set_cmd_list = {
		...
		{	--	show_tradeskill_frame_rank
			'bool',
			"^rank" .. SEPARATOR .. "(.*)" .. SEPARATOR .. "$",
			"show_tradeskill_frame_rank",
			L.SLASH_NOTE["show_tradeskill_frame_rank"],
			NS.ON_SET_CHANGED,
		},
	...
	SlashCmdList["ALATRADEFRAME"] = function(msg)
		msg = strlower(msg);
		--	set
		local _, _, pattern = strfind(msg, "^[ %,%.%_%-%+%=]*set[ %,%.%_%-%+%=]*(.+)%s*$");
		if pattern then
...
do	--	EXTERN SETTING
	function extern_setting.toggle_tradeskill_frame_rank(on)
		SET.show_tradeskill_frame_rank = on;
		NS.ON_SET_CHANGED('show_tradeskill_frame_rank', on);
	end
